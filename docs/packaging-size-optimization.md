...
3. 继续审核 Velopack 的 platform package 输出。macOS 当前同时上传完整安装包和
   `osx-x64`/`osx-arm64` Velopack release feeds；Windows/Linux 由
   `win-x64`/`linux-x64` Velopack release feeds 支持自动更新。
